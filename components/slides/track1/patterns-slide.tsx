      ...
      <div className="max-w-5xl mx-auto w-full">
        ...
        <div className="slide-animate grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8">
          {patterns.map((pattern, index) => (
            <div key={index} className="p-4 border border-neutral-800">
              <h3 className="text-white font-medium text-sm sm:text-base mb-1">{pattern.name}</h3>
              <p className="text-neutral-500 text-xs mb-2">{pattern.desc}</p>
              <p className="text-neutral-600 text-xs italic">Bueno para: {pattern.example}</p>
            </div>
          ))}
        </div>
