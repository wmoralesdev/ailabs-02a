        ...
        <div className="slide-animate grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5 mb-6 sm:mb-8">
          {recipes.map((recipe, index) => (
            ...
            >
              ...
              <h3 className="text-foreground font-medium text-lg sm:text-xl mb-2">{recipe.title}</h3>
              <p className="text-muted-foreground text-xs sm:text-sm mb-4">{recipe.bestFor}</p>

              <div className="space-y-2 text-xs sm:text-sm">
                <p><span className="text-muted-foreground">Usuario:</span> <span className="text-foreground/85">{recipe.user}</span></p>
                <p><span className="text-muted-foreground">Input:</span> <span className="text-foreground/85">{recipe.input}</span></p>
                <p><span className="text-muted-foreground">Steps:</span> <span className="text-foreground/85">{recipe.steps}</span></p>
