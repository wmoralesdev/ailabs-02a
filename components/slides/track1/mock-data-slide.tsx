      ...
      <div className="max-w-5xl mx-auto w-full">
        ...
        <div className="slide-animate grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 mb-6 sm:mb-8">
          {mockItems.map((item, index) => (
            <div key={index} className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 p-4 border border-border">
              <p className="text-muted-foreground text-xs sm:text-sm">{item.real}</p>
              <span className="text-muted-foreground">→</span>
              <p className="text-foreground text-xs sm:text-sm">{item.mock}</p>
            </div>
          ))}
        </div>

        <div className="slide-animate p-4 sm:p-6 border border-foreground/30 bg-foreground/5">
          <p className="text-muted-foreground text-xs uppercase tracking-wider mb-2">Prompt de rescate</p>
