      ...
      <div className="max-w-5xl mx-auto w-full">
        ...
        <div className="slide-animate grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8">
          {beginnerIdeas.map((idea, index) => (
            <div key={index} className="p-4 border border-border hover:border-border transition-colors">
              <h3 className="text-foreground font-medium text-sm sm:text-base mb-1">{idea.title}</h3>
              <p className="text-muted-foreground text-xs sm:text-sm">{idea.desc}</p>
            </div>
          ))}
        </div>
        ...
        <div className="slide-animate mb-6 sm:mb-8">
          <p className="text-muted-foreground text-xs uppercase tracking-wider mb-3">Ideas avanzadas</p>
          <div className="flex flex-wrap gap-2">
            {advancedIdeas.map((idea, index) => (
              <span key={index} className="px-3 py-1.5 bg-secondary text-muted-foreground text-xs sm:text-sm border border-border">
                {idea}
              </span>
            ))}
          </div>
