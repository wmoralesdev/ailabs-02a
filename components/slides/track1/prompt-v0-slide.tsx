  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto w-full">
        ...
        <div className="slide-animate grid grid-cols-1 lg:grid-cols-[1.35fr_0.65fr] gap-4 sm:gap-5 mb-5 sm:mb-6">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {promptParts.map((part, index) => (
              <div key={part.label} className="p-4 sm:p-5 bg-secondary border border-border">
                <div className="font-mono text-muted-foreground text-xs mb-2">
                  {String(index + 1).padStart(2, "0")} / {part.label}
                </div>
                <p className="text-foreground/85 text-sm leading-relaxed">{part.text}</p>
              </div>
            ))}
          </div>

          <div className="p-4 sm:p-5 border border-border bg-background/50">
            <p className="text-muted-foreground text-xs uppercase tracking-wider mb-3">Pistas técnicas</p>
            <div className="flex flex-wrap gap-2 mb-5">
              {technicalCues.map((cue) => (
                <span key={cue} className="px-2 py-1 bg-secondary text-muted-foreground font-mono text-xs border border-border">
                  {cue}
                </span>
              ))}
            </div>
        ...
        {/* Speaker note */}
        <div className="slide-animate p-3 sm:p-4 border-l-2 border-border bg-secondary/30">
        ...
        {/* Builder action */}
        <div className="slide-animate mt-4 p-4 sm:p-5 border border-border bg-secondary/50">
